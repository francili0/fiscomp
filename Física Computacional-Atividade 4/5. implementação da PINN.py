...
T_exact = (T0 - T_amb) * np.exp(-r * t_data) + T_amb
...
# Inicializa modelo, otimizador e pesos
model = PINN()
...
for epoch in range(5000):
    ...
    if epoch % 500 == 0:
        print(f"[{epoch}] Loss total: {loss.item():.4f} | Dados: {loss_data.item():.4f} | Física: {loss_phys.item():.4f}")
...
T_true = (T0 - T_amb) * np.exp(-r * t_test) + T_amb
...
plt.plot(t_test, T_pinn, 'r-', label='PINN (dados + física)')
plt.scatter(t_data, T_obs, color='blue', label='Dados sintéticos', zorder=5)
plt.xlabel('Tempo (s)')
plt.ylabel('Temperatura (°C)')
plt.title('PINN vs Analítica vs Dados Sintéticos')
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.show()
