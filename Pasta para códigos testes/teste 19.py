...
num_samples = 100
angles_train = np.random.uniform(0, 3 * np.pi, num_samples).reshape(-1, 1)  # Reshape to (n_samples, n_features)
...
plt.scatter(angles_train, sin_values_train, label='Dados de treinamento', alpha=0.5)
plt.plot(angles_test, sin_values_true, label='Função Seno Verdadeira', color='blue')
plt.plot(angles_test, sin_values_predicted, label='Interpolação da Rede Neural', color='red')
plt.xlabel(rf'$\theta$')
